...
function read_and_convert_file($fileIn, $fileOut){

	$timeStart = microtime(true);
	$numberProcessed = 0;
	
	$fileOutHandle = fopen($fileOut, "w+");
	if(!$fileOutHandle)	print "Did not open file.\n";
	$prefix = "";
	...
	foreach($decodedData as $elem){
		...
		fwrite($fileOutHandle, $csvFormattedLine);
		$prefix = "\n";
	...
	print "\nTotal Number Processed: $numberProcessed\nTime Elapsed: $elapsed\n\n";
...
/*
	This function takes an array  BowkerID, Title, Author, web_url and ISBN key/value bindings,
...
function clean_and_construct_csv($data){
	$csvParams['a'] = $data['bowkerID'];				//	a references the bowkerID field
	$oldTitle = $data['Title'];
	...
	if (!$cleanAttempt){
		$csvParams['b'] = $oldTitle;					//	b references the title field
		$csvParams['c'] = $oldAuthor;					//	c references the author field
	}
	else{
		$csvParams['b'] = $cleanedData['title'];
		$csvParams['c'] = $cleanedData['author'];
	}

	// Now check if it was a Sunday Review:
	$csvParams['d'] = "";								//	d references the bookReviewURL field
	$csvParams['e'] = "";								//	e references the sundayReviewURL field
	$data['web_url'] = str_replace('\/', '', $data['web_url']); // if there is a backslash character in the url, remove it
	$isSundayReview = check_if_sunday_review($data['web_url']);
	if ($isSundayReview){
		//print "Is Sunday\n";
		$csvParams['e'] = $data['web_url'];
	}
	else{
		//print "Not Sunday\n";
	...
	// Now append the ISBN:
	$csvParams['f'] = $data['ISBN'];					//	f references the Book's ISBN
	...
	if($cleanAttempt){	//	Add the ASIN and Amazon URL to the csv parameter array if we received it
		...
		$csvParams['g'] = "";
		$csvParams['h'] = "";
		$csvParams['g'] = $cleanedData['asin'];			//	g references the asin number
		$csvParams['h'] = $cleanedData['amazonURL'];	//	h references the amazonURL
	}
	...
	ksort($csvParams);	//	Sort the parameters by the key values to stay consistent with the csv format
...
$fileIn = $argv[1];
$fileOut = "cleaned_" . $fileIn;
read_and_convert_file($fileIn, $fileOut);
