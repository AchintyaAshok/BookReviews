...
$JSON_RAW_RESULT_URL = "http://search-add-api-a.prd.use1.nytimes.com/svc/add/v1/lookup.json?_showQuery=true&fq=";
...
/*	This function creates a URL that references the NYTimes ADD Index Entries filtered according the a provided query and
	any additional parameters such as filtering by reference, start date, end date, articles, etc. The function will return
	a URL that will reference the first page of results (an offset parameter will not be provided by default). The results
	will be the Raw Results of the search specified in JSON.	*/
function construct_search_url($query, $filters = NULL){
	global $JSON_RAW_RESULT_URL;
	//$url = $JSON_RAW_RESULT_URL . urlencode($query);
	$url = $JSON_RAW_RESULT_URL;
	$queryPart = rawurlencode($query);
	$queryPart = str_replace('%28', '(', $queryPart);
	$queryPart = str_replace('%29', ')', $queryPart);
	$url .= $queryPart;
