...
require_once('SectionController.interface.php'); 	// Interface from which all Controllers Derive
require('BookController.class.php');			// The Book Controller


class NYTD_GlassBroker_Feeds_GlassBrokerController
{
    private $logger;
    ...
    function __construct($logger)
    {
		...
		$this->sectionHandlers = array(
			...
			//TODO: Implement the Dining and Movies Controllers
			//"dining"	=>	"DiningController",
    ...
    function handleNotification($json, $sentTimestamp, $messageID)
    {
        ...
        // first, make sure this is a desired glass subject
		...
		// TESTING STUFF
			// hardcode some book urls to get glass output for (randomly select an option from an array)
			$bookURLs = array(
				//	This is an array of urls that have some combination of information for title, author & isbn
				"http://www.nytimes.com/2013/07/14/books/review/robert-g-kaisers-act-of-congress.html",
				"http://www.nytimes.com/2013/07/28/books/review/sparta-by-roxana-robinson.html",
				"http://www.nytimes.com/2013/07/21/books/review/rebecca-lees-bobcat.html",
				"http://www.nytimes.com/2011/08/14/books/review/retromania-by-simon-reynolds-book-review.html",
	//			"http://www.nytimes.com/2011/08/14/books/review/rules-of-civility-by-amor-towles-book-review.html"		
	
			);
			// hardcode the values for the handleGlassMessage() function, for testing	
			$messageSection = 'books'; // hardcode the section
			$messageSource = 'scoop';
			$messageType = 'article';
			$randomURL = $bookURLs[rand(0, count($bookURLs)-1)]; // pick a random url
			$messageURL = $randomURL;
		// END TESTING
		
			print "Message Section:\t$messageSection\n";
    ...
    {
        // for the glass message, see what section is being refered to
        ...
        $this->logger->write2Log("Glass notification " . $section . " " . $url,
            NYTD_Feeds_Logger::Debug,
            "GlassBrokerController.class.php:" . __LINE__);


		//	Instantiate a Controller specific for the section and make it handle the SQS Message
		$controllerName = $this->sectionHandlers[$section];
		print "The name of the Controller Class\t$controllerName\n";
		$controller = new $controllerName($url, $glassOutput);
